...
const HeroIntroSection = () => {
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-dental-white to-dental-beige/30">
      <div className="container mx-auto px-4">
        <div className="flex flex-col lg:flex-row items-center gap-16">
          {/* Content */}
          <div className="lg:w-1/2 animate-on-scroll">
            <div className="mb-8">
              <h2 className="mb-6 text-dental-blue leading-tight">
              ...
              <div className="flex flex-wrap gap-4 mb-8">
                {highlights.map((highlight, index) => (
                  <div 
                    key={index}
                    className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border border-dental-turquoise/20 hover:shadow-md transition-all duration-300"
                  >
                    {highlight.icon}
                    <span className="text-sm font-medium text-dental-gray">{highlight.text}</span>
                  </div>
                ))}
              </div>
            </div>
            
            <div className="prose prose-lg max-w-none text-dental-gray space-y-6">
